...
#### 1.3.2.1 集成的外部 Skills ⭐

本项目还集成了以下外部 Skills，用于增强特定场景的能力：

| Skill | 来源 | 用途 | 集成 SOP |
|-------|------|------|----------|
| `dr-jskill` | `.claude/skills/dr-jskill/` | Spring Boot 项目生成、JDTLS 导航、Docker 支持 | sop-scaffold, sop-onboarding |
| `frontend-design` | `.claude/skills/frontend-design/` | 前端界面设计、独特美学、动画效果 | sop-scaffold |
| `tailwind-design-system` | `.claude/skills/tailwind-design-system/` | Tailwind CSS v4 设计系统模式 | sop-scaffold（可选） |

**dr-jskill 核心能力**：
- 自动获取最新 Spring Boot 版本
- JDTLS 代码智能导航（lsp goToDefinition、findReferences 等）
- Docker Compose 自动数据库启动
- GraalVM 原生镜像构建支持
- versions.json 集中版本管理
...
#### 1.3.3.1 多 Agent 并行执行配置 ⭐

各 SOP 支持三种执行模式，适用于不同场景：

| 模式 | 说明 | 适用 SOP | 示例 |
|------|------|----------|------|
| `parallel` | 所有子任务并行执行 | sop-code-review | 格式检查+安全评估+性能分析并行 |
| `hybrid` | 部分并行，部分串行 | sop-incident-response | 收集+复现并行，修复串行 |
| `sequential` | 按顺序执行 | sop-bug-fix | 复现→定位→修复→验证 |

**并行任务配置示例（sop-code-review）**：
```yaml
parallel_tasks:
  - name: 格式检查
...
本项目当前包含 **8 个 SOP**，各自解决不同的问题场景，但它们之间存在一定的关联性：

| SOP 对 | 关系类型 | 说明 |
|--------|----------|------|
| Bug Fix ↔ Incident Response | 相似 | Bug Fix 侧重开发过程修复，Incident Response 侧重生产紧急故障 |
| Code Review ↔ Bug Fix | 嵌入 | Bug Fix 修复后可使用 Code Review 审查代码 |
| Code Review ↔ Incident Response | 嵌入 | 故障修复后可使用 Code Review 审查代码 |
| Library Research ↔ Onboarding | 依赖 | 入职前可先用 Library Research 了解技术栈 |
| Scaffold ↔ Onboarding | 依赖 | 新项目生成后可使用 Onboarding 熟悉 |

**Scaffold 新角色**：
- 生成项目后可触发 Onboarding 熟悉项目
- 生成代码后可直接触发 Code Review 审查
- 支持 dr-jskill 集成，自动配置 JDTLS
...
**场景四：新项目初始化（dr-jskill 集成）**⭐。使用 Scaffold 生成项目（可选 dr-jskill 脚本），Onboarding 熟悉项目，Code Review 审查生成的代码。

### 2.7 SOP 增强特性 ⭐

基于 dr-jskill 和多 Agent 并行能力，各 SOP 已增强以下特性：

#### 2.7.1 JDTLS 代码智能导航

Java 相关 SOP（sop-bug-fix, sop-code-review, sop-scaffold, sop-onboarding, sop-incident-response）支持 JDTLS：

| 任务 | JDTLS 命令 | 传统方式 |
|------|-----------|----------|
| 查找定义 | `lsp goToDefinition` | grep 搜索 |
| 查找引用 | `lsp findReferences` | grep 搜索 |
| 查看类型/文档 | `lsp hover` | 阅读源码 |
| 重命名 | `lsp rename` | 手动替换 |

**优先级建议**：`lsp` → `grep` with `.java` glob → `view`

#### 2.7.2 dr-jskill 集成

**sop-scaffold** 集成了 dr-jskill 能力：
- 使用 `scripts/create-project-latest.mjs` 自动生成 Spring Boot 项目
- 自动获取最新 Spring Boot 版本
- 内置 JDTLS 配置
- Docker Compose 自动数据库启动
- versions.json 集中版本管理
...
**sop-scaffold** 集成了 frontend-design 能力：
- 独特字体选择（非 Inter/Roboto/Arial）
- Cohesive 颜色主题设计
- CSS 优先动画原则
- 设计意图布局（非千篇一律）
- 可选 Tailwind CSS v4 集成
...
| sop-incident-response | hybrid | 收集+代码分析+监控分析并行 |
| sop-bug-fix | parallel | 代码搜索+代码分析并行 |

#### 2.7.5 编码规范增强 ⭐

**sop-code-review** 增强了 Lombok 使用规范检查：**全面使用，统一规范**
...
**sop-scaffold** 增强了服务验收的测试支持：

**测试数据生成**：
- SQL 初始化数据（`sql/test-data.sql`）
- Mock 测试数据（Spring Boot `@Sql` 注解）
- 必须包含：正常用户、禁用用户、边界用户
